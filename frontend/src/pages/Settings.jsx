    ...
    <Layout title="Settings">
      <div className="max-w-4xl space-y-10">
        {settingsGroups.map((group) => (
          <div key={group.title}>
            <h3 className="text-sm font-bold uppercase tracking-widest text-neutral-400 mb-6 px-1">{group.title}</h3>
            <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 overflow-hidden shadow-sm">
              {group.items.map((item, idx) => (
                <div 
                  key={item.name} 
                  className={`flex items-center justify-between p-6 ${idx !== group.items.length - 1 ? 'border-b border-outline-variant/5' : ''} hover:bg-surface-container-low/30 transition-colors`}
                >
                  <div className="space-y-1">
                    <p className="font-semibold text-on-surface">{item.name}</p>
                    <p className="text-xs text-on-surface-variant max-w-md">{item.description}</p>
                  </div>
                  ...
                  <div>
                    ...
                    {item.type === 'select' && (
                      <div className="flex items-center gap-2 text-xs font-bold text-primary bg-primary/5 px-4 py-2 rounded-lg cursor-pointer hover:bg-primary/10 transition-colors">
                        {item.value}
                        <span className="material-symbols-outlined text-sm">expand_more</span>
                      </div>
                    )}
                    {item.type === 'button' && (
                      <button className="text-xs font-bold text-white bg-black px-6 py-2 rounded-lg hover:opacity-80 transition-opacity">
                        {item.label}
                      </button>
                    )}
                  </div>
        ...
        <div className="pt-10 flex gap-4">
          <button className="bg-primary text-on-primary px-8 py-3 rounded-xl font-bold hover:opacity-90 transition-all active:scale-95 text-sm">
            Save Changes
          </button>
          <button className="text-neutral-500 px-8 py-3 rounded-xl font-bold hover:bg-neutral-100 transition-all text-sm">
            Reset to Default
          </button>
        </div>
      </div>
